...
const EDIT_SUBCOMMANDS: [(&str, &str); 53] = [
    ...
    (
        "set-table-props",
        "표 속성(칸간격·여백·글자처럼·배치·테두리/배경·캡션 등) — --table/--props (JSON)",
    ),
    (
        "set-section-def",
        "구역 정의(머리말·꼬리말·바탕쪽 감추기·시작 번호 등) — --props (JSON) [--section]",
    ),
    (
        "move-table",
        "표 위치 오프셋 이동 — --table/--dh/--dv (HWPUNIT)",
    ),
    (
        "transpose-table",
        "표 행/열을 제자리에서 바꾸기 — --table (병합 셀 있으면 거부)",
    ),
    (
        "set-column-widths",
        "표 열 폭 절대값 설정 — --table/--widths (쉼표 구분 HWPUNIT, 열 수와 일치)",
    ),
    ...
    (
        "apply-char-format",
        "본문 문단 글자 범위에 글자 서식 적용 — --props 필수, --section/--para/--offset/--count",
    ),
    (
        "apply-para-format",
        "본문 문단에 문단 서식 적용 — --props 필수, --section/--para (생략 시 0)",
    ),
    (
        "apply-style",
        "본문 문단에 스타일 적용 — --style 필수, --section/--para (생략 시 0)",
    ),
    (
        "apply-para-format-in-hf",
        "머리말/꼬리말 문단에 문단 서식 적용 — --header|--footer/--props 필수",
    ),
    (
        "apply-para-format-in-footnote",
        "각주/미주 안 문단에 문단 서식 적용 — --section/--para/--ctrl/--props 필수",
    ),
    (
        "apply-endnote-shape",
        "구역의 미주 모양(번호 서식·구분선 등) 적용 — --props 필수, --section (생략 시 0)",
    ),
    (
        "add-bookmark",
        "책갈피 추가 — --name 필수, --section/--para/--offset (생략 시 0)",
    ),
    (
        "delete-bookmark",
        "책갈피 삭제 — --section/--para/--ctrl (책갈피 컨트롤 인덱스) 모두 필수",
    ),
    (
        "rename-bookmark",
        "책갈피 이름 변경 — --section/--para/--ctrl/--name 모두 필수",
    ),
    (
        "delete-table",
        "본문 최상위 표 삭제 — --table (표 번호) 필수",
    ),
    (
        "insert-page-break",
    ...
    (
        ...
        "문단 번호 다시 시작 — --mode 필수(0/1/2), --count(=시작 번호, mode=2 전용)/--section/--para (생략 시 0/0/1)",
    ),
    (
        "insert-image",
