...
pub(crate) fn run_edit(args: &[String]) -> i32 {
    ...
    match args.first().map(String::as_str) {
        ...
        // [upstream #5185/#5192 계열 선별 이식] 표 단위 편집 4종 + 구역 정의 — 코어는
        // 기존 네이티브 함수 재사용, CLI 배선만 신규.
        ...
        // [upstream #5185/#5192 계열 선별 이식, "문단 기본" 배치] 코어는 기존
        // text_editing.rs 네이티브 함수 재사용, CLI 배선만 신규.
        ...
        // [upstream #5185/#5192 계열 선별 이식, "서식/스타일" 배치] 코어는 기존
        // formatting.rs/header_footer_ops.rs/footnote_ops.rs/object_ops/shape.rs
        // 네이티브 함수 재사용, CLI 배선만 신규.
        ...
        // [upstream #5185/#5192 계열 선별 이식, "책갈피/구조" 배치] 코어는 기존
        // bookmark_query.rs/table_ops.rs/text_editing.rs 네이티브 함수 재사용, CLI
        // 배선만 신규.
...
/// `edit set-section-def` — 구역 정의(SectionDef: 머리말/꼬리말/바탕쪽/테두리/채움
/// 감추기, 쪽·그림·표·수식 시작 번호, 단 간격, 기본 탭 간격 등)를 고친다 (upstream
/// #5185/#5192 계열 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존 `set_section_def_native`
/// 재사용). 원래 report(2026-09-01)는 이 항목에 "신규 IR 필요"로 적었으나 착수 전
/// 선확인(2026-09-02)에서 rhwp-code에 이미 동형 네이티브 함수가 있음을 확인했다 —
/// set-table-props와 같은 결론.
///
/// `--section`은 생략하면 0(첫 구역). `--props`는 JSON 객체 문자열이며 다루는 속성은
/// 그 필드 이름으로 정한다(예: `{"hideHeader":true}`). 지원 필드 전체는
/// `apply_section_def_json`(`document_core/queries/rendering.rs`) 구현이 정본이다 — 이
/// CLI 배선은 필드를 해석하지 않고 그대로 넘긴다. JSON 객체가 아니거나 구역 번호가
/// 범위를 벗어나면 `--dry-run`에서도 사용법/실행 오류로 미리 잡는다.
pub(crate) fn edit_set_section_def(args: &[String]) -> i32 {
    const USAGE: &str = "사용법: rhwp edit set-section-def <파일> --props <JSON> [--section <번호>] [-o <출력>] [--dry-run] [--verify] [--json]";
    ...
    // resolve_table_index와 같은 이유로 --dry-run 에서도 먼저 범위를 검사한다.
    if section >= doc.document().sections.len() {
        eprintln!(
            "오류: 구역 {} 이 범위를 벗어났습니다 (구역 수 {}).",
            section,
    ...
    if !dry_run {
        let result = match doc.set_section_def_native(section, &props) {
            Ok(v) => v,
            Err(e) => {
        ...
        page_count = serde_json::from_str::<serde_json::Value>(&result)
            .ok()
            .and_then(|v| v["pageCount"].as_u64());
    }
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_secdef.{}", stem, out_format.ext())
    });
    ...
    if json_mode {
        ...
        if let Some(pc) = page_count {
            envelope["pageCount"] = serde_json::json!(pc);
        }
        if !dry_run {
            envelope["output"] = serde_json::Value::String(output_path.clone());
            envelope["outputFormat"] = serde_json::Value::String(out_format.label().to_string());
    ...
    if dry_run {
        println!(
            "구역 정의 변경 예정: {} 구역{} props={}",
            file_path, section, props
        );
    } else {
        println!(
            "구역 정의 변경 완료: {} → {} — 구역{} props={}",
            file_path, output_path, section, props
        );
    }
    if verify_failed {
        eprintln!("검증 실패(--verify): 저장본 재파싱 IR 차이 — 상세는 --json 또는 ir-diff");
...
/// `edit insert-footnote` — 각주를 삽입한다 (upstream #5185/#5192 계열 선별 이식 —
/// 배선은 CLI뿐, 코어 로직은 기존 `insert_footnote_native` 재사용).
///
/// `--section`/`--para`/`--offset`은 생략하면 0.
pub(crate) fn edit_insert_footnote(args: &[String]) -> i32 {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_footnote.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        println!(
            "각주 삽입 예정: {} 구역{} 문단{} 오프셋{}",
            file_path, section, para, offset
        );
    } else {
        println!(
            "각주 삽입 완료: {} → {} — 구역{} 문단{} 오프셋{}",
            file_path, output_path, section, para, offset
        );
    }
    if verify_failed {
        eprintln!("검증 실패(--verify): 저장본 재파싱 IR 차이 — 상세는 --json 또는 ir-diff");
...
/// `edit insert-endnote` — 미주를 삽입한다 (upstream #5185/#5192 계열 선별 이식 —
/// 배선은 CLI뿐, 코어 로직은 기존 `insert_endnote_native` 재사용).
///
/// `--section`/`--para`/`--offset`은 생략하면 0.
pub(crate) fn edit_insert_endnote(args: &[String]) -> i32 {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_endnote.{}", stem, out_format.ext())
    });
...
/// `edit delete-footnote` — 각주/미주를 삭제한다 (upstream #5185/#5192 계열 선별
/// 이식 — 배선은 CLI뿐, 코어 로직은 기존 `delete_footnote_native` 재사용).
///
/// `--section`/`--para`/`--ctrl` 모두 필수 — 기본값이 없다(잘못된 컨트롤을 실수로
/// 지우지 않도록).
pub(crate) fn edit_delete_footnote(args: &[String]) -> i32 {
    ...
    let (Some(file_path), Some(section), Some(para), Some(ctrl)) =
        (file_path, section_arg, para_arg, ctrl_arg)
    else {
        eprintln!("{USAGE}");
        return EXIT_USAGE;
    };
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_delfn.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        println!(
            "각주 삭제 예정: {} 구역{} 문단{} 컨트롤{}",
            file_path, section, para, ctrl
        );
    } else {
        println!(
            "각주 삭제 완료: {} → {} — 구역{} 문단{} 컨트롤{}",
            file_path, output_path, section, para, ctrl
        );
    }
    if verify_failed {
        eprintln!("검증 실패(--verify): 저장본 재파싱 IR 차이 — 상세는 --json 또는 ir-diff");
...
/// `edit insert-footnote-text` — 각주/미주 안 문단에 텍스트를 삽입한다 (upstream
/// #5185/#5192 계열 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존
/// `insert_text_in_footnote_native` 재사용).
///
/// `--ctrl`(각주/미주 컨트롤 인덱스)와 `--text`는 필수. `--section`/`--para`/
/// `--fn-para`(각주 내부 문단 인덱스)/`--offset`은 생략하면 0.
pub(crate) fn edit_insert_footnote_text(args: &[String]) -> i32 {
        ...
        match args[i].as_str() {
            ...
            "--fn-para" => {
                ...
                match args.get(i).and_then(|v| v.parse::<usize>().ok()) {
                    Some(v) => fn_para = v,
                    None => {
                        eprintln!("오류: --fn-para 뒤에 0 이상의 정수가 필요합니다.");
                        return EXIT_USAGE;
                    }
                }
            }
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_fntext.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        println!(
            "각주 텍스트 삽입 예정: {} 구역{} 문단{} 컨트롤{}",
            file_path, section, para, ctrl
        );
    } else {
        println!(
            "각주 텍스트 삽입 완료: {} → {} — 구역{} 문단{} 컨트롤{}",
            file_path, output_path, section, para, ctrl
        );
    }
    if verify_failed {
...
/// `edit delete-text-in-footnote` — 각주/미주 안 문단 텍스트를 삭제한다 (upstream
/// #5185/#5192 계열 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존
/// `delete_text_in_footnote_native` 재사용).
///
/// `--count`(1 이상)는 필수, 나머지 좌표는 생략하면 0.
pub(crate) fn edit_delete_text_in_footnote(args: &[String]) -> i32 {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_fndeltxt.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        println!(
            "각주/미주 텍스트 삭제 예정: {} 구역{} 문단{} 컨트롤{} 각주문단{} 오프셋{} 글자{}",
            file_path, section, para, ctrl, fn_para, offset, count
        );
    } else {
        println!(
            "각주/미주 텍스트 삭제 완료: {} → {} — 구역{} 문단{} 컨트롤{} 각주문단{} 오프셋{} 글자{}",
            file_path, output_path, section, para, ctrl, fn_para, offset, count
        );
    }
    if verify_failed {
        eprintln!("검증 실패(--verify): 저장본 재파싱 IR 차이 — 상세는 --json 또는 ir-diff");
...
/// `edit split-paragraph-in-footnote` — 각주/미주 안 문단을 분할한다 (upstream
/// #5185/#5192 계열 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존
/// `split_paragraph_in_footnote_native` 재사용). CLI에서는 `restore_meta`를 항상
/// `None`으로 넘긴다(undo 전용 축이며 이 명령의 범위 밖).
pub(crate) fn edit_split_paragraph_in_footnote(args: &[String]) -> i32 {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_fnsplit.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        println!(
            "각주/미주 문단 분할 예정: {} 구역{} 문단{} 컨트롤{} 각주문단{} 오프셋{}",
            file_path, section, para, ctrl, fn_para, offset
        );
    } else {
        println!(
            "각주/미주 문단 분할 완료: {} → {} — 구역{} 문단{} 컨트롤{} 각주문단{} 오프셋{}",
            file_path, output_path, section, para, ctrl, fn_para, offset
        );
    }
    if verify_failed {
        eprintln!("검증 실패(--verify): 저장본 재파싱 IR 차이 — 상세는 --json 또는 ir-diff");
...
/// `edit merge-paragraph-in-footnote` — 각주/미주 안 문단을 이전 문단과 병합한다
/// (upstream #5185/#5192 계열 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존
/// `merge_paragraph_in_footnote_native` 재사용).
///
/// `--fn-para`는 생략하면 1(0은 "첫 문단은 병합 불가"로 항상 거부되므로 기본값에서
/// 제외했다) — 명시적으로 0을 주면 사용법 오류로 미리 잡는다.
pub(crate) fn edit_merge_paragraph_in_footnote(args: &[String]) -> i32 {
    const USAGE: &str = "사용법: rhwp edit merge-paragraph-in-footnote <파일> [--section N] [--para N] [--ctrl N] [--fn-para N(1 이상)] [-o <출력>] [--dry-run] [--verify] [--json]";
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--fn-para" => {
                ...
                match args.get(i).and_then(|v| v.parse::<usize>().ok()) {
                    Some(0) | None => {
                        eprintln!("오류: --fn-para 는 1 이상의 정수가 필요합니다.");
                        return EXIT_USAGE;
                    }
                    Some(v) => fn_para = v,
                }
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_fnmerge.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        println!(
            "각주/미주 문단 병합 예정: {} 구역{} 문단{} 컨트롤{} 각주문단{}",
            file_path, section, para, ctrl, fn_para
        );
    } else {
        println!(
...
/// `edit insert-text` — 본문 문단 좌표에 텍스트를 삽입한다 (upstream #5185/#5192
/// 계열 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존 `insert_text_native` 재사용).
///
/// `--section`/`--para`/`--offset`은 생략하면 0. 다른 5개와 달리 이 명령은
/// `--dry-run`에서도 구역·문단·오프셋 범위를 미리 검사한다(upstream 원본 그대로 —
/// 본문 좌표 오류를 조기에 잡기 위함).
pub(crate) fn edit_insert_text(args: &[String]) -> i32 {
    ...
    let para_chars = doc.document().sections[section].paragraphs[para]
        .text
        .chars()
        .count();
    if offset > para_chars {
        eprintln!("오류: --offset 이 문단 길이를 넘습니다 (문단 길이 {para_chars}): {offset}");
        return EXIT_USAGE;
    }

    if !dry_run {
        if let Err(e) = doc.insert_text_native(section, para, offset, text) {
...
/// `edit delete-text` — 본문 문단 좌표의 텍스트를 삭제한다 (upstream #5185/#5192
/// 계열 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존 `delete_text_native` 재사용).
///
/// `--count`(1 이상)는 필수, 나머지 좌표는 생략하면 0.
pub(crate) fn edit_delete_text(args: &[String]) -> i32 {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_deltext.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        println!(
            "텍스트 삭제 예정: {} 구역{} 문단{} 오프셋{} 글자{}",
            file_path, section, para, offset, count
        );
    } else {
        println!(
            "텍스트 삭제 완료: {} → {} — 구역{} 문단{} 오프셋{} 글자{}",
            file_path, output_path, section, para, offset, count
        );
    }
    if verify_failed {
        eprintln!("검증 실패(--verify): 저장본 재파싱 IR 차이 — 상세는 --json 또는 ir-diff");
...
/// `edit insert-paragraph` — 빈 문단을 삽입한다(앞 문단 서식을 상속) (upstream
/// #5185/#5192 계열 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존
/// `insert_paragraph_native` 재사용).
///
/// `--section`/`--para`는 생략하면 0. 다른 4개(삭제/병합/분할)와 달리 이 명령은
/// `--dry-run`에서도 구역·문단 범위를 미리 검사한다(upstream 원본 그대로).
/// `--para`는 문단 수와 같은 값(맨 끝에 추가)까지 허용한다.
pub(crate) fn edit_insert_paragraph(args: &[String]) -> i32 {
    ...
    if para > para_count {
        eprintln!(
            "오류: --para 이 범위를 벗어났습니다 (구역 {section} 문단 0~{para_count}): {para}"
        );
        return EXIT_USAGE;
    }

    if !dry_run {
        if let Err(e) = doc.insert_paragraph_native(section, para) {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_paragraph.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        println!(
            "문단 삽입 예정: {} 구역{} 문단{}",
            file_path, section, para
        );
    } else {
        println!(
            "문단 삽입 완료: {} → {} — 구역{} 문단{}",
            file_path, output_path, section, para
        );
    }
    if verify_failed {
        eprintln!("검증 실패(--verify): 저장본 재파싱 IR 차이 — 상세는 --json 또는 ir-diff");
...
/// `edit delete-paragraph` — 문단을 삭제한다(마지막 남은 문단은 거부) (upstream
/// #5185/#5192 계열 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존
/// `delete_paragraph_native` 재사용).
///
/// `--section`/`--para`는 생략하면 0.
pub(crate) fn edit_delete_paragraph(args: &[String]) -> i32 {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_delpara.{}", stem, out_format.ext())
    });
    ...
    // [upstream 원본] 삭제된 문단 인덱스는 더 이상 없으므로 이전 문단 기준으로
    // changedPages를 계산한다.
    let changed_pages = if dry_run {
        serde_json::Value::Null
    } else {
        match doc.pages_covering_paragraphs(&[(section, para.saturating_sub(1))]) {
            Some(pages) => serde_json::json!(pages),
            None => serde_json::Value::Null,
        }
    };
...
/// `edit merge-paragraph` — 문단을 이전 문단과 병합한다 (upstream #5185/#5192
/// 계열 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존 `merge_paragraph_native` 재사용).
///
/// `--section`/`--para`는 생략하면 0. `--para`가 가리키는 문단이 `--para`−1과
/// 합쳐진다(첫 문단 지정 시 병합 대상이 없어 런타임 오류).
pub(crate) fn edit_merge_paragraph(args: &[String]) -> i32 {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_mergepara.{}", stem, out_format.ext())
    });
    ...
    // [upstream 원본] para 는 para-1 로 흡수되므로 changedPages는 흡수 대상(para-1)
    // 기준으로 계산한다.
    let changed_pages = if dry_run {
        serde_json::Value::Null
    } else {
...
/// `edit split-paragraph` — 문단을 지정 오프셋에서 분할한다 (upstream #5185/#5192
/// 계열 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존 `split_paragraph_native` 재사용).
/// CLI에서는 `restore_meta`를 항상 `None`으로 넘긴다(undo 전용 축이며 이 명령의
/// 범위 밖).
///
/// `--section`/`--para`/`--offset`은 생략하면 0.
pub(crate) fn edit_split_paragraph(args: &[String]) -> i32 {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_splitpara.{}", stem, out_format.ext())
    });
...
/// `edit apply-char-format` — 본문 문단 글자 범위에 글자 서식을 적용한다 (upstream
/// #5185/#5192 계열 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존
/// `apply_char_format_native` 재사용).
///
/// `--section`/`--para`/`--offset`은 생략하면 0, `--count`를 생략하면 문단 끝까지.
/// 범위(구역/문단/오프셋)는 `--dry-run`에서도 미리 검사한다(upstream 원본 그대로).
pub(crate) fn edit_apply_char_format(args: &[String]) -> i32 {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--props" => {
                i += 1;
                match args.get(i) {
                    Some(v) if !v.is_empty() => props = Some(v.clone()),
                    _ => {
    ...
    if dry_run {
        println!(
            "글자 서식 적용 예정: {} 구역{} 문단{} 오프셋{}",
            file_path, section, para, offset
        );
    } else {
        println!(
            "글자 서식 적용 완료: {} → {} — 구역{} 문단{} 오프셋{}",
            file_path, output_path, section, para, offset
        );
    }
    if verify_failed {
...
/// `edit apply-para-format` — 본문 문단에 문단 서식을 적용한다 (upstream
/// #5185/#5192 계열 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존
/// `apply_para_format_native` 재사용).
///
/// `--section`/`--para`는 생략하면 0. 범위는 `--dry-run`에서도 미리 검사한다.
pub(crate) fn edit_apply_para_format(args: &[String]) -> i32 {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_pfmt.{}", stem, out_format.ext())
    });
...
/// `edit apply-style` — 본문 문단에 스타일을 적용한다 (upstream #5185/#5192 계열
/// 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존 `apply_style_native` 재사용).
///
/// `--style`(스타일 인덱스)은 필수, `--section`/`--para`는 생략하면 0. 범위는
/// `--dry-run`에서도 미리 검사한다.
pub(crate) fn edit_apply_style(args: &[String]) -> i32 {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--style" => {
                i += 1;
                match args.get(i).and_then(|v| v.parse::<usize>().ok()) {
                    Some(v) => style_arg = Some(v),
                    None => {
                        eprintln!("오류: --style 뒤에 0 이상의 정수가 필요합니다.");
                        return EXIT_USAGE;
                    }
                }
            }
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_style.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        println!(
            "스타일 적용 예정: {} 구역{} 문단{} 스타일{}",
            file_path, section, para, style_id
        );
    } else {
        println!(
            "스타일 적용 완료: {} → {} — 구역{} 문단{} 스타일{}",
            file_path, output_path, section, para, style_id
        );
    }
    if verify_failed {
        eprintln!("검증 실패(--verify): 저장본 재파싱 IR 차이 — 상세는 --json 또는 ir-diff");
...
/// `edit apply-para-format-in-hf` — 머리말/꼬리말 문단에 문단 서식을 적용한다
/// (upstream #5185/#5192 계열 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존
/// `apply_para_format_in_hf_native` 재사용).
///
/// `--header`/`--footer` 중 정확히 하나와 `--props`는 필수. `--section`/
/// `--apply-to`/`--para`는 생략하면 0. upstream 원본과 동일하게 `--props`의
/// JSON 형식은 사전 검사하지 않는다(코어가 그대로 파싱).
pub(crate) fn edit_apply_para_format_in_hf(args: &[String]) -> i32 {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_hfpfmt.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        println!(
            "{} 문단 서식 적용 예정: {} 구역{}",
            kind, file_path, section
        );
    } else {
        println!(
            "{} 문단 서식 적용 완료: {} → {} — 구역{}",
            kind, file_path, output_path, section
        );
    }
    if verify_failed {
        eprintln!("검증 실패(--verify): 저장본 재파싱 IR 차이 — 상세는 --json 또는 ir-diff");
...
/// `edit apply-para-format-in-footnote` — 각주/미주 안 문단에 문단 서식을 적용한다
/// (upstream #5185/#5192 계열 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존
/// `apply_para_format_in_footnote_native` 재사용).
///
/// `--section`/`--para`/`--ctrl`(각주/미주 컨트롤 인덱스)/`--props`는 필수,
/// `--fn-para`(각주/미주 내부 문단)는 생략하면 0. upstream 원본과 동일하게
/// `--props`의 JSON 형식은 미리 검사한다.
pub(crate) fn edit_apply_para_format_in_footnote(args: &[String]) -> i32 {
    ...
    let (Some(file_path), Some(section), Some(para), Some(ctrl), Some(props)) =
        (file_path, section_arg, para_arg, ctrl_arg, props)
    else {
        eprintln!("{USAGE}");
        return EXIT_USAGE;
    };
    ...
    if !dry_run {
        ...
        {
            eprintln!("오류: 각주/미주 문단 서식 적용 실패 - {}", e);
            return EXIT_RUNTIME;
        }
    }
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_fnpfmt.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        println!(
            "각주/미주 문단 서식 적용 예정: {} 구역{} 문단{} 컨트롤{}",
            file_path, section, para, ctrl
        );
    } else {
        println!(
            "각주/미주 문단 서식 적용 완료: {} → {} — 구역{} 문단{} 컨트롤{}",
            file_path, output_path, section, para, ctrl
        );
    }
    if verify_failed {
...
/// `edit apply-endnote-shape` — 구역의 미주 모양(번호 서식·구분선 등)을 적용한다
/// (upstream #5185/#5192 계열 선별 이식 — 배선은 CLI뿐, 코어 로직은 기존
/// `apply_endnote_shape_native` 재사용).
///
/// `--props`는 필수, `--section`은 생략하면 0. upstream 원본과 동일하게
/// `--props`의 JSON 형식은 사전 검사하지 않는다(코어가 알려진 키만 골라 읽는다).
pub(crate) fn edit_apply_endnote_shape(args: &[String]) -> i32 {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_enshape.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        println!("미주 모양 적용 예정: {} 구역{}", file_path, section);
    } else {
        println!(
            "미주 모양 적용 완료: {} → {} — 구역{}",
            file_path, output_path, section
        );
    }
    if verify_failed {
        eprintln!("검증 실패(--verify): 저장본 재파싱 IR 차이 — 상세는 --json 또는 ir-diff");
...
/// `edit add-bookmark` — 책갈피를 추가한다 (upstream #5185/#5192 계열 선별 이식,
/// "책갈피/구조" 배치 — 배선은 CLI뿐, 코어 로직은 기존 `add_bookmark_native` 재사용).
///
/// `--name`은 필수, `--section`/`--para`/`--offset`은 생략하면 0. 네이티브 함수는
/// 성공/실패를 `Err`가 아니라 반환 JSON의 `{"ok":true}`/`{"ok":false,"error":...}`로
/// 알린다(이름 비어있음·중복 이름은 `ok:false`, 구역/문단 범위 초과만 `Err`) —
/// upstream 원본과 동일하게 **`--dry-run`에서는 네이티브를 호출하지 않으므로 구역/
/// 문단 범위를 미리 검사하지 않는다**.
pub(crate) fn edit_add_bookmark(args: &[String]) -> i32 {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_addbm.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        println!(
            "책갈피 추가 예정: {} 이름 {} 구역{} 문단{} 오프셋{}",
            file_path, name, section, para, offset
        );
    } else {
        println!(
            "책갈피 추가 완료: {} → {} — 이름 {}",
...
/// `edit delete-bookmark` — 책갈피를 삭제한다 (upstream #5185/#5192 계열 선별 이식,
/// "책갈피/구조" 배치 — 배선은 CLI뿐, 코어 로직은 기존 `delete_bookmark_native` 재사용).
///
/// `--section`/`--para`/`--ctrl`(책갈피 컨트롤 인덱스) 모두 필수. 네이티브 함수는
/// `add-bookmark`와 동일하게 반환 JSON의 `ok`/`error`로 실패를 알린다(해당 컨트롤이
/// 책갈피가 아니면 `ok:false`) — **`--dry-run`에서는 네이티브를 호출하지 않으므로
/// 구역/문단/컨트롤 범위를 미리 검사하지 않는다**.
pub(crate) fn edit_delete_bookmark(args: &[String]) -> i32 {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_delbm.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        ...
        println!("책갈피 삭제 완료: {} → {}", file_path, output_path);
    }
    if verify_failed {
        eprintln!("검증 실패(--verify): 저장본 재파싱 IR 차이 — 상세는 --json 또는 ir-diff");
        process::exit(3);
...
/// `edit rename-bookmark` — 책갈피 이름을 바꾼다 (upstream #5185/#5192 계열 선별
/// 이식, "책갈피/구조" 배치 — 배선은 CLI뿐, 코어 로직은 기존 `rename_bookmark_native`
/// 재사용).
///
/// `--section`/`--para`/`--ctrl`/`--name` 모두 필수. 네이티브 함수는 반환 JSON의
/// `ok`/`error`로 실패를 알린다(이름 비어있음·중복 이름·컨트롤이 책갈피가 아님은
/// `ok:false`) — **`--dry-run`에서는 네이티브를 호출하지 않으므로 구역/문단/컨트롤
/// 범위를 미리 검사하지 않는다**.
pub(crate) fn edit_rename_bookmark(args: &[String]) -> i32 {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_renbm.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        println!(
            "책갈피 이름 변경 예정: {} 구역{} 문단{} 컨트롤{} → {}",
            file_path, section, para, ctrl, name
        );
    } else {
        println!("책갈피 이름 변경 완료: {} → {}", file_path, output_path);
    }
    if verify_failed {
        eprintln!("검증 실패(--verify): 저장본 재파싱 IR 차이 — 상세는 --json 또는 ir-diff");
        process::exit(3);
...
/// `edit delete-table` — 본문 최상위 표를 삭제한다 (upstream #5185/#5192 계열 선별
/// 이식, "책갈피/구조" 배치 — 배선은 CLI뿐, 코어 로직은 기존
/// `delete_table_control_native` 재사용).
///
/// `--table`(표 번호)은 필수. 기존 `resolve_table_index`로 좌표를 해석하므로
/// **표 번호 범위 초과는 `--dry-run`에서도 잡힌다**(delete-row/delete-col과 동일).
/// 네이티브 함수 자체는 `Err`만 반환한다(책갈피 3종과 달리 JSON `ok`/`error` 래핑
/// 없음).
pub(crate) fn edit_delete_table(args: &[String]) -> i32 {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_deltable.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        println!("표 삭제 예정: {} 표{}", file_path, table_no);
    } else {
        println!(
            "표 삭제 완료: {} → {} — 표{}",
            file_path, output_path, table_no
        );
    }
    if verify_failed {
        eprintln!("검증 실패(--verify): 저장본 재파싱 IR 차이 — 상세는 --json 또는 ir-diff");
...
/// `edit insert-page-break` — 커서 위치에서 문단을 분할하고 강제 쪽 나누기를
/// 삽입한다 (upstream #5185/#5192 계열 선별 이식, "책갈피/구조" 배치 — 배선은
/// CLI뿐, 코어 로직은 기존 `insert_page_break_native` 재사용).
///
/// `--section`/`--para`/`--offset`은 생략하면 0. upstream 원본과 동일하게
/// **`--section`/`--para` 범위는 `--dry-run`에서도 미리 검사한다**(네이티브 호출
/// 전에 무조건 검사) — 단 `--offset`은 문단 길이 대비 사전 검사하지 않는다(upstream도
/// 안 함).
pub(crate) fn edit_insert_page_break(args: &[String]) -> i32 {
    ...
    if !dry_run {
        if let Err(e) = doc.insert_page_break_native(section, para, offset) {
            eprintln!("오류: 쪽 나누기 삽입 실패 - {}", e);
            return EXIT_RUNTIME;
        }
    }
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_pagebreak.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        println!(
            "쪽 나누기 삽입 예정: {} 구역{} 문단{} 오프셋{}",
            file_path, section, para, offset
        );
    } else {
        println!("쪽 나누기 삽입 완료: {} → {}", file_path, output_path);
    }
    if verify_failed {
        eprintln!("검증 실패(--verify): 저장본 재파싱 IR 차이 — 상세는 --json 또는 ir-diff");
        process::exit(3);
...
/// `edit insert-column-break` — 커서 위치에서 문단을 분할하고 강제 단 나누기를
/// 삽입한다 (upstream #5185/#5192 계열 선별 이식, "나머지" 배치 — 배선은 CLI뿐,
/// 코어 로직은 기존 `insert_column_break_native` 재사용).
///
/// `--section`/`--para`/`--offset`은 생략하면 0. `insert-page-break`(직전 배치)와
/// 완전히 같은 패턴 — upstream 원본과 동일하게 **`--section`/`--para` 범위는
/// `--dry-run`에서도 미리 검사한다**(네이티브 호출 전에 무조건 검사) — 단
/// `--offset`은 문단 길이 대비 사전 검사하지 않는다(upstream도 안 함).
pub(crate) fn edit_insert_column_break(args: &[String]) -> i32 {
    ...
    if !dry_run {
        if let Err(e) = doc.insert_column_break_native(section, para, offset) {
            eprintln!("오류: 단 나누기 삽입 실패 - {}", e);
            return EXIT_RUNTIME;
        }
    }
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_colbreak.{}", stem, out_format.ext())
    });
...
/// `edit split-paragraph-in-hf` — 머리말/꼬리말 문단을 분할한다 (upstream
/// #5185/#5192 계열 선별 이식, "나머지" 배치 — 배선은 CLI뿐, 코어 로직은 기존
/// `split_paragraph_in_header_footer_native` 재사용).
///
/// `--header`/`--footer` 중 정확히 하나는 필수, `--section`/`--apply-to`/`--para`/
/// `--offset`은 생략하면 0. `restore_meta`는 undo 전용 축이라 CLI에서는 항상
/// `None`으로 넘긴다(`split-paragraph`/`split-paragraph-in-footnote`와 같은 관례,
/// 이 명령 범위 밖). upstream 원본과 동일하게 **`--dry-run`에서 구역/문단/오프셋
/// 범위를 미리 검사하지 않는다**(네이티브 호출 자체가 `--dry-run`에서 생략됨).
pub(crate) fn edit_split_paragraph_in_hf(args: &[String]) -> i32 {
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_hfsplit.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        ...
        println!("{} 문단 분할 완료: {} → {}", kind, file_path, output_path);
    }
    if verify_failed {
        eprintln!("검증 실패(--verify): 저장본 재파싱 IR 차이 — 상세는 --json 또는 ir-diff");
        process::exit(3);
...
/// (upstream #5185/#5192 계열 선별 이식, "나머지" 배치 — 배선은 CLI뿐, 코어
/// 로직은 기존 `merge_paragraph_in_header_footer_native` 재사용).
///
/// `--header`/`--footer` 중 정확히 하나는 필수. `--para`는 생략하면 1(0은
/// "첫 문단은 병합 불가"로 항상 거부되므로 기본값에서 제외했다 — upstream 원본과
/// 같은 관례, `merge-paragraph-in-footnote`도 동일) — 명시적으로 0을 주면 코어가
/// 거부한다(exit 1). upstream 원본과 동일하게 **`--dry-run`에서 구역/문단 범위를
/// 미리 검사하지 않는다**.
pub(crate) fn edit_merge_paragraph_in_hf(args: &[String]) -> i32 {
    const USAGE: &str = "사용법: rhwp edit merge-paragraph-in-hf <파일> --header|--footer [--section N] [--apply-to 0|1|2] [--para N(1 이상)] [-o <출력>] [--dry-run] [--verify] [--json]";
    ...
    let output_path = out_path.unwrap_or_else(|| {
        let stem = Path::new(file_path)
            ...
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| "output".to_string());
        format!("{}_hfmerge.{}", stem, out_format.ext())
    });
    ...
    if dry_run {
        ...
        println!("{} 문단 병합 완료: {} → {}", kind, file_path, output_path);
    }
    if verify_failed {
        eprintln!("검증 실패(--verify): 저장본 재파싱 IR 차이 — 상세는 --json 또는 ir-diff");
        process::exit(3);
