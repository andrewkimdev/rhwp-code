...
fn two_inline_tac_tables_stay_on_one_line() {
    ...
    assert!(
        ...
        "둘째 TAC 표가 x={second_x:.1}px 에 있다 — #6601 가로 회귀. \
         선행 컨트롤 수를 `offsets[0] / 8` 로 세면 빈 세그먼트가 과하게 앞서 \
         두 표 사이 공백이 뒤로 밀리고 x 가 236px 로 돌아간다 \
         (허용 {SECOND_TABLE_X_MIN_PX:.1}~{SECOND_TABLE_X_MAX_PX:.1}px, 한/글 280px)"
    );
}
