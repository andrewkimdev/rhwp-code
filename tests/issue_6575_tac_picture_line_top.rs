...
fn issue_6575_tac_picture_sits_on_line_top_when_line_is_taller() {
    ...
    // 대상: 폭 557.25px 의 TAC 그림 (원본 5쪽 스크린샷 자리). rhwp-code 는 이 축소
    // 픽스처에서 문단 상단 여백 계산이 upstream 과 8px 갈라져(이 수정과 무관한 기존
    // 차이) 줄 상단이 436.68 이다 — upstream 문서 값 444.68 을 그대로 쓰지 않는다.
    // 디버그로 실측 확인(box_h=283.13 ≈ raw_lh=283.13 > baseline=240.67 → `.max(y)`
    // 클램프로 줄 상단에 정확히 앉음, 결함 시 baseline 바닥 맞춤인 +35px 더 내려간
    // 471.68).
    ...
    assert!(
        (y - 436.68).abs() < 1.5,
        "TAC 그림 상단({y:.2})이 줄 상단(436.68, rhwp-code 실측) 에 있어야 한다 — \
         결함 시 471.68 (baseline 바닥 맞춤, bl−h=+35px)"
    );
}
