...
//! 구역은 `write_section` 이 템플릿 치환으로 처리하지만 subList(셀) 경로에는 그
//! 보완이 없어, 셀 안 구역이 `hp:secPr` 없이 뒤따르는 `hp:colPr` 만 남았다.
//!
//! 실물 피해: upstream 코퍼스 HWP5 6,491문서를 레코드 단위로 전수해 보면 BodyText
//! 스트림 수보다 `secd` 가 많은 문서가 3건이고 그 17개가 전부 표 셀 안(level 3)이다.
//! 그중 06874 는 셀 안 `secd` 15개를 잃어 한글이 `부록 4-④` 문단부터 문서 끝까지
//! 폐기했다(204→159쪽, −57,525자). 한글 2022 오라클 주입 검정에서 그 자리 **한 곳에만**
//! `secPr` 를 되살리면 쪽수·글자수가 원본과 정확히 같아졌다.
//!
//! 계약: 셀 문단이 `Control::SectionDef` 를 들고 있으면 그 셀의 `hp:subList` 안에
//! `hp:secPr` 가 나와야 한다. 본문 최상위 구역은 종전대로 유지된다.
//!
//! upstream(edwardkim/rhwp) 커밋 `d7f90eb00`(#5873)의 통합 테스트를 이식했다.
