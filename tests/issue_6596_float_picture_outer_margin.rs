//! Issue #6596 (upstream): 본문 부동 그림의 바깥 여백이 잉크 위치에 반영되지 않아
//! 여백만큼 좌·상으로 어긋나던 결함의 가드.
//!
...
//! 어긋났다 (upstream samples↔pdf 215문서 실측, 여백>0 45건 중 44건).
//!
//! 세 표본이 규칙을 고정한다 (96DPI px, 한컴 PDF 이미지 bbox 대조):
//!
//! ```text
//! hwp3-sample5-hwp5   4쪽 pi=74  Square       Paper/Paper  왼쪽 정렬  → (62.85, 87.43)
//! hwp3-sample-hwp5    3쪽 pi=41  TopAndBottom Column/Para  왼쪽 정렬  → (124.73, 143.52)
//! [2027] 온새미로     35쪽 pi=7  Square       Column/Para  오른쪽 정렬 → (393.23, 165.9)
//! ```
//!
//! 오른쪽 정렬 표본은 여백이 left 2mm·right 0·top 0 이라 잉크가 움직이지 않아야
//! 한다 — 여백을 상자 크기로 정렬에 넣지 않고 원점에만 더하면 이 표본이 깨진다.
