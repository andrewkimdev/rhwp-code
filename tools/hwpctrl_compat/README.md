...
- **시간 제한과 정리.** `run_gate.py` 가 시나리오마다 제한을 걸고, 끝나면 남은
  `Hwp.exe`/`HwpFrame.exe` 를 정리한다.
- **오라클은 한글2022(12.x)로 고정한다.** `run_gate.py` 의 기본값이며, 어긋나면 **시나리오를
  아예 돌리지 않고** `<id>.rejected.json` 만 남긴다(정답지 자리를 오염시키지 않는다).
