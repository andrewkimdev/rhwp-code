...
const BASELINE: Readonly<Record<string, number>> = {
  ...
  'src/command/commands/template-ops.ts': 16, // hwpx-template-engine 마커 authoring 연산부(template.ts에서 분리 이관 — 호출부 변화 없음, 키만 이동): template:tag-selection(splitTable×2 + insertTableRow + mergeTableCells + deleteTextInCell + insertTextInCell) / template:clear-marker(deleteTableRow) — 전부 executeOperation snapshot 안에서 호출 (undo 기록됨) / +3: 마커 행 시각 스타일 강제(applyCharFormatInCell·applyParaFormatInCell·setCellProperties, applyMarkerRowStyle) / +1: 마커 행 높이 22px 고정(resizeTableCells, 세로 가운데 정렬은 같은 setCellProperties 호출에 얹음) / +1: 마커 행 글꼴을 이름이 아니라 fontId 로만 적용할 수 있어 findOrCreateFontId 로 등록·조회(#HEADER/#REPEAT-TITLE 등 마커 행 서체 강제 일관화) / +3: #PAGENO 태깅 시 예약 누름틀(현재_페이지/전체_페이지) 자동 삽입 — ensurePageNoFields가 mergeTableCells(마지막 행 다열 병합) + insertTextInCell(' / ' 구분자) + insertClickHereField(두 필드를 배열 루프 1개 호출부에서 삽입, field-suggest.ts와 같은 관례) 추가, 전부 같은 executeOperation snapshot 안(undo 기록됨) / +1: 계산 필드(#seq:/#sum:) 삽입 — insertComputedField가 insertClickHereField 1회 호출, template:insert-computed-field 커맨드의 executeOperation snapshot 안(undo 기록됨)
  ...
  'src/engine/input-handler.ts': 13, // +1: 누름틀 제거 이관 시 removeFieldAt 이 양식모드(직접 유지)/일반모드(snapshot) 두 분기로 분리 / +1: Edit 오버레이 커밋이 셀 내부는 setFormValueInCell 로 분기(CheckBox 와 동일 조건 — 기존 flat 호출은 표 컨트롤 슬롯을 가리켜 실패) / +1: 셀 블록 글자 서식이 applyCharFormatInCell 을 executeOperation snapshot 안에서 호출(여러 셀에 걸친 글자 서식 커맨드 부재) / +1: 중첩 셀 블록 글자 서식도 applyCharFormatInCellByPath 를 같은 snapshot 안에서 호출 / +2: 머리말·꼬리말(applyParaFormatInHf)과 각주(applyParaFormatInFootnote) 문단 서식 배선 — 둘 다 snapshot 라우팅 / -7: 양식 개체 오버레이 클러스터(클릭/라디오/콤보/편집)를 input-handler-form-overlay.ts 로 이관 — 호출부 변화 없음, 7곳 표면이 키로 이동 / -5: 객체 배치·그리기 모드 클러스터(드롭 그림 삽입·다각형·글상자/도형 배치)를 input-handler-placement.ts 로 이관 — 호출부 변화 없음, 5곳 표면이 키로 이동 / -2: 누름틀·클릭히어 탐색 클러스터(커서 이동·누름틀 지우기·경계 진출입·마커 갱신)를 input-handler-field-nav.ts 로 이관 — 호출부 변화 없음, removeFieldAt 두 분기(양식 직접/일반 snapshot) 표면이 키로 이동 / -4: 글자·문단 서식 클러스터(캐럿 대기 글자모양·셀 블록 서식·문단 서식 배선)를 input-handler-format.ts 로 이관 — 호출부 변화 없음, applyCharFormatInCell·applyCharFormatInCellByPath·applyParaFormatInHf·applyParaFormatInFootnote 4곳 표면이 키로 이동(스냅샷 라우팅 유지)
  'src/engine/input-handler-connector.ts': 1,
  'src/engine/input-handler-field-nav.ts': 2, // input-handler.ts 누름틀·클릭히어 탐색 클러스터 이관 분리 — removeFieldAt×2 (양식 모드 방어적 직접 분기 + 일반 모드 snapshot 라우팅, 라우팅 변화 없음)
  ...
  'src/engine/input-handler-format.ts': 4, // input-handler.ts 글자·문단 서식 클러스터 이관 분리 — applyCharFormatInCell + applyCharFormatInCellByPath (셀/중첩셀 블록, snapshot 라우팅 유지) + applyParaFormatInHf + applyParaFormatInFootnote (머리말·꼬리말/각주 문단 서식, snapshot 라우팅 유지)
  'src/engine/input-handler-keyboard.ts': 21,
  'src/engine/input-handler-mouse.ts': 3,
  'src/engine/input-handler-picture.ts': 11,
  'src/engine/input-handler-placement.ts': 5, // input-handler.ts 객체 배치·그리기 모드 클러스터 이관 분리 — insertPicture + setCellPicturePropertiesByPath + setPictureProperties (드롭 그림 snapshot 라우팅 유지) + createShapeControl×2 (다각형·글상자/도형, 라우팅 변화 없음)
