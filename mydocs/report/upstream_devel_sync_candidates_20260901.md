...
| `ba097d6bf` (#5185), `e0851908b` (#5192) 등 | feat(cli): 편집 명령 13건/29건 통합 | `src/cli/commands/edit/*`, `tests/cases/*_contract.rs` | `edit` 서브커맨드 42개+ 신규(`set-section-def`, `set-table-props`, `move-table`, `transpose-table`, `set-column-widths` 등). feature parity 가치는 크지만 rhwp-code CLI 구조가 이미 다르게 쪼개져 있어 명령 단위 개별 선별 이식을 권장. **진행(2026-09-02)**: `move-table`/`transpose-table`/`set-column-widths` 3건은 rhwp-code에 이미 있던 네이티브 함수(`table_ops.rs`)에 CLI 배선만 추가해 이식 완료 — 나머지(`set-table-props` 선확인 필요, `set-section-def` 신규 IR 필요, ~37개 미검토)는 다음 라운드 대상. |
