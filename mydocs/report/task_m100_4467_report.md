...
- 공통 문서 엔진, Web/WASM 기반, 공식 브라우저·VS Code 확장, npm 패키지와 범용 CLI·MCP·API
  계약은 업스트림 범위다.
...
- `python3 scripts/check_markdown_links.py --changed-from upstream/devel --forbid-redirect-references`
  - 556개 문서, 변경 파일 18개, redirect stub 30개, 내부 상대 링크 이상 없음
...
Open PR을 생성한 뒤 프로젝트 PR workflow에 따라 리뷰어 지정 또는 maintainer self-review 경로를
결정하고, 최신 head의 CI 결과를 확인한 후 merge 여부를 판정한다.
