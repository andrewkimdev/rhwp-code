...
지적 5건을 전부 검증해 사실로 확인하고 ①②③④⑤를 반영했다. 캐시 총량이 이미 무료 한도의 100.1%라
...
2.50GB`를 그대로 처리했다. 논증뿐 아니라 실제로 스위치가 죽어 있었다.
...
workflow에서 추출한 스크립트에 실제 캐시 53개, branch 3건, tag 23건, 열린 PR 13건을 넣어 dry-run으로
돌렸다.

| 항목 | 값 |
| --- | --- |
| 정리 전 | 53개 / 10.01GB (한도의 100.1%) |
| 고아 ref | 23개 / 2.50GB |
| 구 세대 | 3개 / 1.17GB |
| 정리 후(추정) | **27개 / 6.34GB (63.4%)** |
...
- 예상치는 `before - staleBytes` 계산값이다. merge 뒤 실제 cron 스윕 1회로 확인하고 새 기준선을
  #4080에 기록한다.
- `devel` 한정 KEEP=1은 이번 범위 밖이다. `refs/heads/devel`만 19개 5.84GB지만 #3684가 KEEP=2를 택한
