...
동일 794×1123 해상도의 독립 Studio Canvas와 PDF 물리 p10–p17 재대조에서 p10, p11,
p15, p17은 표 소유·frame·문단 경계가 유지된다. 이후 확대 대조에서 p11의 첫 줄이
render tree에는 있으나 실제 `Body`/`Cell` clip 위(y=105.1, clip 시작 y=113.4/117.1)에
있어 사라지는 것을 확인했다. 따라서 이 단계는 p11의 상단 clip과 그 뒤 p12–p17 경계를
같은 PDF 물리 페이지 기준으로 복원한다.
...
clip 안으로 이동했고, PDF처럼 `제50조의4`보다 먼저 보인다. p10–p17의 page owner는
변경하지 않는다.
...
결과는 8 passed / 0 failed다. 추가한 focused assertion은 p11의 첫 continuation 줄이
physical nested-cell clip 안(y=117–140)에 위치해야 한다고 검증한다. `export-svg`의
`overflowCellLines` 원장은 이 대형 RowBreak 표의 clip 밖 논리 source를 계속 기록하므로,
단독 성공 지표로 사용하지 않았고 PNG/PDF 1:1 대조와 함께 판정했다.

## 결론

p10–p17의 이번 경계 결함은 해결했다. 글꼴 raster 차이는 pagination/owner 수정의
근거로 사용하지 않았으며, 다른 페이지의 별도 fidelity 후보는 다음 stage에서 독립적으로
판정한다.
