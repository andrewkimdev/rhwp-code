...
# Task #3820 Stage 18 — 현재 macOS visual sweep p10–p17 PDF 직접 대조
...
- visual sweep: `scripts/visual_sweep.py --rhwp-bin <현재 release CLI> --pages 10-17 --dpi 144`.
  스크립트가 기본 `export-svg`와 render tree를 문서 전체에 대해 새로 만들고, 선택한 8쪽만
  144dpi `rsvg-convert` SVG와 Poppler PDF raster로 대조했다.
- 현재 default SVG와 PDF는 각각 17쪽이다. p10–p17의 compare·overlay·review가 모두 생성됐다.
...
8쪽의 pixel match 평균은 87.88%지만, 종이 여백이 큰 비중을 차지한다. 내용 잉크만 본
자동 일치율 보조값은 평균 8.91%(최저 p17 7.96%, 최고 p12 10.12%)로 낮다. 따라서 이 수치는
페이지 수가 같거나 넓은 여백이 같다는 사실로 layout fidelity를 통과 처리할 수 없음을 뒷받침한다.

![p10 review: rhwp(왼쪽), PDF(가운데), overlay(오른쪽)](../pr/assets/task_m100_3820_stage18_current_macos_font_validation/visual_sweep/review/review_010.png)

코멘트: 내용 픽셀 중심 자동 일치율 보조값 = 약 9.57%.
높을수록 좋음: 기준 PDF와 rhwp PNG가 더 비슷함
낮을수록 나쁨/검토 필요: 잉크 위치나 형태 차이가 큼
단, 사람 판정 정확도가 아니라 내용 픽셀 중심 자동 일치율 보조값입니다.

각 페이지의 비교 자료는 다음 위치에 보관한다.

- compare: `mydocs/pr/assets/task_m100_3820_stage18_current_macos_font_validation/visual_sweep/compare/compare_010.png` … `compare_017.png`
- overlay: `mydocs/pr/assets/task_m100_3820_stage18_current_macos_font_validation/visual_sweep/overlay/overlay_010.png` … `overlay_017.png`
- review: `mydocs/pr/assets/task_m100_3820_stage18_current_macos_font_validation/visual_sweep/review/review_010.png` … `review_017.png`
- 수치/후보 원장: `visual_sweep/overlay/overlay_metrics.json`, `visual_sweep/analysis/metrics.json`, `visual_sweep/manifest.json`
...
p10–p17을 통과하지 못했다. 다음 구현은 p10–p12의 table continuation·line-flow 분기,
p10–p15의 빈 사각 marker→순번 변환, 전 페이지의 세로 기준선 drift를 source→IR→layout→SVG
